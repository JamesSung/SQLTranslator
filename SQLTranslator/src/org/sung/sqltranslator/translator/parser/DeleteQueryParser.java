...
public class DeleteQueryParser extends AbstractQueryParser {
	private static Log log = LogFactory.getLog(DeleteQueryParser.class);

	private static String[] removeKeywds = { "DELETE", "FROM" };
	...
	List<Table> extractTable(String sql) {
		...
		String table = null;
		if (whereIdx > 0) {
			table = sql.substring(0, whereIdx).trim();
		} else {
			table = sql.trim();
		}
